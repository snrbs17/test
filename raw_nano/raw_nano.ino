...
#define smallDelay delay(1)
#define middleDelay delay(10)
#define bigDelay delay(100)
...
void transmit(byte mode, byte pin, byte value){//     transmit start     transmit start     transmit start
  ...
  smallDelay;//this is the time CE pin must be HIGH for before going back into RX mode
...
void get_data(){// get data start get data start get data start get data start get data start
  ...
  if(data_in[1]==1){//data starting with '1' sets up the pinmode
    ...
    middleDelay;//very important delay - this lets the transmitter finish 
...
void NRF_ping(){//    start ping    start ping    start ping    start ping    start ping    start ping
  int ping;
  ping=random(16);//get a random byte
  Serial.print("Pinging with     ");
  Serial.print(ping);
  
  transmit(6, 0, ping);// send teh ping out  starting with 6
  middleDelay;//give it a little
...
void NRFpinmode(byte pin, byte mode){// start pin mode     start pin mode     start pin mode
  int i;
  for(i=0; i<maxRetry; i++){//We give it 10 tries to set the pinmode
  transmit(1, pin, mode);//send out with '1' the pin and the mode
  middleDelay;//give a little time to respond
  if(data2==pin && mode ==data3)//if we get the right data back,
  i=maxRetry;//force out of the for loop
 
  if(i==maxRetry-1)//if we ever get too far along, at ==9, then we never set the pin
...
byte NRFpinread(byte pin){//   start pin read   start pin read   start pin read   start pin read   start pin read
  int i;
  for(i=0; i<maxRetry; i++){//same as teh other routines here
